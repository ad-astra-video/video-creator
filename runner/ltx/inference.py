...
class VideoCreatorInferenceEngine:
    ...
    def __init__(
        ...
        self._zimage_pipe = None  # ZImagePipeline, lazily built
    ...
    def _evict_zimage(self) -> None:
        """Drop the image pipeline from GPU. Called before the video model loads."""
        if self._zimage_pipe is not None:
            logger.info("Evicting Z-Image pipeline from GPU before loading video model")
    ...
    def read_video_frames(video_path: str):
        ...
        for frame in container.decode(stream):
            rgb = frame.to_rgb().to_ndarray(format="float32") / 255.0
            frames.append(rgb)
        container.close()
