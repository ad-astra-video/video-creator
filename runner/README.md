...
The runner:
1. Loads LTX-2.3 distilled models into GPU VRAM
2. Registers itself with a Livepeer Orchestrator via `register_runner()`
3. Exposes HTTP endpoints for video generation (T2V, I2V, A2V)
4. Receives inference requests from the Orchestrator (proxied from LTX-Desktop)
...
```bash
docker compose -f docker/docker-compose.runner.yml up
```
