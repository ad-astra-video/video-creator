...
class ExtendHandler(StateHandlerBase):
    ...
    def run(self, req: ExtendRequest) -> ExtendResponse:
        ...
        if self.state.app_settings.remote_inference_enabled:
            return self._run_livepeer_extend(
                video_file=video_file, duration=duration, prompt=prompt, mode=mode
            )
    ...
    def _run_livepeer_extend(
        ...
        *,
        ...
        # Get fps from source video for frame count
        fps = 24.0
        try:
            _, _, _, _ = read_source_metadata(str(video_file))
        except Exception:
            pass
        extend_frames = round(duration * fps)
